struct CircArr
    ...
    function CircArr(mkp::Markup)
        ...
        if _mean(vcat(day_arr_in_hour, sleep_arr_in_hour)) < 30
            return new(0, 0, 0, 0, false, "undefined")
        end
        ...
        return new(day_var, sleep_var, arr_day, arr_sleep, rel, res)
    end
end
...
function Base.show(io::IO, res::CircArr)
    ...
    println(io, "Отношение дневной СВ к ночной: ", res.day_var / res.sleep_var)

    if res.result == "day"
        println(io, "Различия СВ достоверны, тип аритмии определен как дневной")
    elseif res.result == "night"
